...
namespace SimpleSAML\Module\moodle\Controller;
...
class External
{
    ...
    public function authpage(Request $request)
    {
         ...
         */
        $this->authState::loadState(urldecode($matches[1]), 'moodle:External');
        ...
        if ($request->getMethod() === 'POST') {
            ...
            // Defer to remote moodle the authentication ? 
        ...
        $t = new Template($this->config, 'moodle:authenticate.twig');
    ...
    public function redirecttest(Request $request): RunnableResponse
    {
        ...
        $state = $this->authState::loadState($stateId, 'moodle:redirectfilter-test');
